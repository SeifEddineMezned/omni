...
// VideoTransition is handled via /video route

const Login = () => {
  const { dispatch } = useAppContext();
  const navigate = useNavigate();
  // video playback is triggered by navigation to /video
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Navigate to video route which will play the transition then go to dashboard
        navigate("/video");
      } else {
        setError(
          result.message ||
